...
//
// 架构: Electron 在 127.0.0.1:58713-58723 起 WS server, 插件 background service worker
...
//   ws://127.0.0.1:<bridge-port> 任何浏览器网页都连得上 (localhost 不受 mixed-content 限制), 裸放行 = 恶意网页
...
function createBridgeServers() {
  const server = http.createServer((req, res) => {
    if (req.url === "/health") {
      ...
      res.end(JSON.stringify({
        ...
        host: HOST,
        port: server.address()?.port || currentPort || PORT_START,
      }));
      return;
    }
    res.writeHead(404);
  ...
  const wsServer = new WebSocketServer({ server, path: "/agent" });
...
async function start(userDataDir) {
  ...
  for (let port = PORT_START; port <= PORT_END; port += 1) {
    const candidate = createBridgeServers();
    httpServer = candidate.server;
    wss = candidate.wsServer;

    try {
      await listenCurrentServer(port);
      currentPort = port;
      break;
    } catch (e) {
      lastError = e;
      cleanupCurrentServer();
      if (e?.code !== "EADDRINUSE") throw e;
    }
  }

  if (!wss || !httpServer || !currentPort) {
    throw lastError || new Error(`端口范围 ${HOST}:${PORT_START}-${PORT_END} 无可用端口`);
  }
...
async function listenCurrentServer(port) {
