...
长 Shot 不再默认从开头裁切。程序先依据已对齐的分段事件、字幕、人物和说话人证据边界生成固定来源范围，再让 Planner 只选择不可修改的 `candidateId`。
...
- 该范围内重新裁切后的事件、字幕、人物、说话人和 `alignedSegments`。
...
- 长 Shot 优先选择最大时长内最靠后的事件、字幕、人物或说话人边界。
- 没有合适证据边界时按最大时长切分。
