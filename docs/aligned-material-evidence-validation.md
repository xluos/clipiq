...
将同一 Shot 内原本分散的事件语义、字幕、人物出镜和说话人轨迹按真实素材时间边界对齐，让 Planner 不再自行猜测“某一秒发生什么、谁在画面里、谁在说什么”。
...
- 微秒级 `startUs / endUs`。
- 当前时间段的事件描述；分析节点提供了内部边界时标为 `eventGranularity: "segment"`，否则以 `"shot"` 明确降级。
- 当前时间片内重叠的字幕和字幕粒度。
...
- `event@segment` 表示分析节点提供了 Shot 内部事件边界，可用于具体时间段。
- `event@shot` 表示只有整 Shot 描述，避免伪装为更细粒度的视觉理解。
...
- 分段事件语义来自已有 `AnalysisNode.startSec/endSec` 与 Shot 的交集，不额外编造时间；节点不能切出 Shot 内部边界时仍降级为 Shot 级。
- 当前没有额外运行逐帧视觉语义模型，因此 `event@segment` 的精度上限仍由主分析节点时间范围决定。
