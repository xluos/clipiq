...
type PersonAppearance = {
  ...
  confidence: number;
  thumbnailUrl?: string;
  focusBounds?: {           // 归一化人脸轨迹联合框，供可追溯重构图使用
    x: number;
    y: number;
    width: number;
    height: number;
...
- [x] 将完整 Vlog 质量集与人物身份 bootstrap 集分开验收；人物集通过不能替代横竖屏、事件角色和负样本完整的粗剪固定集。
- [x] 用本机既有素材建立首个人物身份 bootstrap：18 条 / 25.2 分钟，主人物跨 8 条素材并含 10 个不同人物负例；文件与覆盖校验通过。
- [x] 用生产 YuNet → SFace → Tracker → Assignment 链路标定跨素材身份阈值：`0.5` 下同人物 pair 召回 `75%`、错误合并 `0`、精确率 `100%`；侧脸低头保持独立簇。
...
- [ ] 确认首个目标剪映版本及其安装环境。
- [x] M4 通过前，不将某个第三方草稿库写死为正式依赖；当前依赖中未加入 `capcut-cli` / `pyJianYingDraft`，正式 exporter 仍由真机报告门禁锁定。
